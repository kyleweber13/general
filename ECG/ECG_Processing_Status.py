...
def check_whats_processed():
    ...
    df.insert(loc=0, column='full_id', value=[f"{row.study_code}_{row.subject_id}_{row.coll_id}" for row in df.itertuples()])
    ...
    print(df_summary[['weber_nonwear', 'smital', 'CN_beats', 'CN_events', 'last_checked']].loc[0])
