...
def run_algorithm(ecg_signal: list or np.array,
                  ...
                  amplitude_thresh: int or float = 250,
    ...
    df_snr_template = df_snr_q1.loc[(df_snr_q1['avg_snr'] >= 25) & (df_snr_q1['duration'] >= 60)]
    df_snr_template.sort_values('avg_snr', ascending=False).reset_index(drop=True)

    qrs, qrs_n, qrs_crop = create_beat_template_snr_bouts(df_snr=df_snr_template.iloc[0:20 if df_snr_template.shape[0] >= 20 else df_snr_template.shape[0]],
                                                          ecg_signal=ecg_signal, plot_data=False,
                                                          sample_rate=sample_rate, peaks=data_dict['original']['idx'],
                                                          window_size=.75, remove_outlier_amp=True,
                                                          use_median=False, peak_align=.4, remove_mean=True)

    print("\nRunning peak screening algorithm ==================================")

    start_peak_idx = find_first_highly_correlated_beat(ecg_signal=ecg_signal, peaks=data_dict['original']['idx'],
                                                       sample_rate=sample_rate, template=qrs_crop,
                                                       correl_window_size=correl_window_size,
    ...
    df_last = "quality_screen"  # key for most recently processed df
    ...
    df_last = 'nonwear_screen'  # key for most recently processed df
    ...
    if orphanidou_dfs is None:
        orphanidou = run_orphanidou(signal=ecg_signal, peaks=data_dict['valid']['idx'], timestamps=raw_timestamps,
                                    window_size=.3, sample_rate=sample_rate, epoch_len=epoch_len,
...
#dfs_orph = import_orphanidou_dfs(full_id, root_dir="W:/NiMBaLWEAR/OND09/analytics/ecg/signal_quality/orphanidou/")
dfs_orph = None
...
"""
export_orphanidou_dfs(full_id=full_id, data_dict=data,
                      use_keys=['orph_epochs', 'orph_valid', 'orph_invalid', 'orph_bout'],
                      root_dir="W:/NiMBaLWEAR/OND09/analytics/ecg/signal_quality/orphanidou/")
"""
...
fig = plot_results(data_dict=data, ecg_signal=filt, ecg_timestamps=ecg.ts, subj=full_id,
                   ...
                   smital_raw=ecg.snr, df_nw=ecg.df_nw, ds_ratio=3,
                   )
